    ...
    *,
    ...
    for node_id, node in graph.items():
        ...
        if class_type == "LoadImage" and "image" in inputs:
            ...
            patched["ref_image"].append(str(node_id))
    ...
    if not patched["prompt"]:
        raise RuntimeError("Flux2 ref workflow patch failed: no CLIPTextEncode text node found")
    if not patched["ref_image"]:
        raise RuntimeError("Flux2 ref workflow patch failed: no LoadImage image node found")
    if not patched["filename_prefix"]:
        raise RuntimeError("Flux2 ref workflow patch failed: no SaveImage filename_prefix node found")

    print(
        "[flux2_ref.patch] "
        f"prompt_nodes={patched['prompt']} "
        f"ref_nodes={patched['ref_image']} "
